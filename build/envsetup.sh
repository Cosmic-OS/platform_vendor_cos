...
function mka() {
    ...
    if [ -f $ANDROID_BUILD_TOP/$QTI_BUILDTOOLS_DIR/build/update-vendor-hal-makefiles.sh ]; then
        ...
        vendor_hal_script=$ANDROID_BUILD_TOP/vendor/cos/scripts/vendor_hal_makefile_generator.sh
        regen_needed=1
    fi
    ...
    if [ $regen_needed -eq 1 ]; then
        ...
        RET=$?
        if [ $RET -ne 0 ]; then
            echo -n "${color_failed}#### HAL file .bp generation failed due to incomaptible HAL files , please check above error log"
            echo " ####${color_reset}"
            return $RET
        fi
    fi
