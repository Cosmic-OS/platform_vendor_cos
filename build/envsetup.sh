function __print_cos_functions_help() {
cat <<EOF
...
- githubremote:    Add git remote for Cosmic-OS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Cosmic model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/cosmic-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/Cosmic-OS/$PROJECT
...
function repopick() {
    T=$(gettop)
    $T/vendor/cos/build/tools/repopick.py $@
}
