...
function setup_vendor() {
    ...
    export COSMIC_ROOT="$3"
    if [ ! -d "$COSMIC_ROOT" ]; then
        echo "\$COSMIC_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$COSMIC_ROOT/$OUTDIR" ]; then
        mkdir -p "$COSMIC_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$COSMIC_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$COSMIC_ROOT"/"$OUTDIR"/Android.mk
    export ANDROIDBP="$COSMIC_ROOT"/"$OUTDIR"/Android.bp
    export BOARDMK="$COSMIC_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "$COMMENT_PREFIX Copyright (C) $YEAR The Cosmic-OS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "$COMMENT_PREFIX Copyright (C) 2017-$YEAR The Cosmic-OS Project\n" >> $1
        else
            printf "$COMMENT_PREFIX Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The Cosmic-OS Project\n" >> $1
        fi
    else
        printf "$COMMENT_PREFIX Copyright (C) $YEAR The Cosmic-OS Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$COSMIC_ROOT"/vendor/cos/build/tools/smali/baksmali.jar
        export SMALIJAR="$COSMIC_ROOT"/vendor/cos/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$COSMIC_ROOT"/vendor/cos/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$COSMIC_ROOT"/vendor/cos/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$COSMIC_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$COSMIC_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$COSMIC_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$COSMIC_ROOT"/vendor/cos/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$COSMIC_ROOT"/"$OUTDIR"/radio
