...
func init() {
	android.RegisterModuleType("cos_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cosExpandVariables(ctx, dep_file)
	...
	cmd := cosExpandVariables(ctx, String(g.properties.Cmd))
